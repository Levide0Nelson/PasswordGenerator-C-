...
// Pour la conversion du mot de passe en minuscule pour la conversion quelque soit la caste
string toLower(const string& keyword)
{
    string upperKeyword = keyword;
    transform(upperKeyword.begin(), upperKeyword.end(), upperKeyword.begin(), [](unsigned char c) {return tolower(c);});
    return upperKeyword;
}
    ...
    if (fichier.is_open())
    {
        ...
        while (getline(fichier, ligne))
        {
            ...
            if (blocEnCours)
            {
                ...
                if ((ligne.find("Contexte de création : ") != string::npos && ligne.find(toLower(keyword)) != string::npos) || (ligne.find("Créé le") != string::npos && ligne.find(toLower(keyword)) != string::npos))
